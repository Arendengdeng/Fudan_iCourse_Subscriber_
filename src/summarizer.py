...
4. **公式规范：绝对禁止使用 $$ 或 $ 符号。必须把所有的 $$ 或 $ 形式的公式全部替换并改写为反引号包裹的行内代码块（即 `...`）。** 所有数学公式、物理量或科学变量必须直接写在 Markdown 的行内代码块中（例如：`E=mc^2` 或 `\frac{-b \pm \sqrt{b^2-4ac}}{2a}`）。这样做的目的是为了确保公式以纯文本代码形式展示，严禁触发任何图片或 LaTeX 渲染逻辑。

5. **忠于原文与详略得当**：总结必须详略得当，长度适宜（例如，对于90分钟长度的课程，总结长度应为3000－4000字左右；135分钟的课程，应为5000字左右。输出和输入的长度压缩比例在1:8左右为宜），包含具体的推导细节、案例、文献或者核心概念，不要过度概括，用完整连贯的段落来表示老师的意思。禁止捏造录音中未提及的内容。
